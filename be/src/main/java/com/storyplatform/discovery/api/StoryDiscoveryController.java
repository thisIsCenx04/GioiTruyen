...
public class StoryDiscoveryController {
    ...
    private List<HomeStorySummaryResponse> storiesByLabel(
        ...
        List<HomeStorySummaryResponse> manual = manualStoriesByLabel(
                label,
                orderByLatestChapter
        );
        return manual.isEmpty()
                ? automaticStoriesByLabel(label, orderByLatestChapter)
                : manual;
    }

    private List<HomeStorySummaryResponse> manualStoriesByLabel(
            String label,
            boolean orderByLatestChapter
    ) {
    ...
    private List<HomeStorySummaryResponse> automaticStoriesByLabel(
            String label,
            boolean orderByLatestChapter
    ) {
        String sortColumn = orderByLatestChapter
                ? "COALESCE(latest.latest_chapter_at, s.updated_at, s.published_at)"
                : "COALESCE(s.published_at, s.created_at)";
        String condition = switch (label) {
            case "NEW_RELEASE" ->
                    "COALESCE(s.published_at, s.created_at) >= DATE_SUB(UTC_TIMESTAMP(6), INTERVAL 7 DAY)";
            case "RECENT_UPDATE" ->
                    "COALESCE(latest.latest_chapter_at, s.updated_at, s.published_at) >= DATE_SUB(UTC_TIMESTAMP(6), INTERVAL 7 DAY)";
            case "ORIGINAL" -> "s.origin = 'ORIGINAL'";
            case "COMPLETED" -> "s.completion_status = 'COMPLETED'";
            case "EXCLUSIVE" -> "COALESCE(rp.exclusive, FALSE) = TRUE";
            default -> "TRUE";
        };
        List<HomeStorySummaryResponse> stories = jdbc.sql("""
                        SELECT s.id,
                        ...
                        ORDER BY %s DESC,
                                 ...
                                 s.title ASC
                        LIMIT 8
                        """.formatted(condition, sortColumn))
                .query((result, rowNumber) -> story(result))
                .list();
        return stories.isEmpty()
                ? fallbackPublishedStories(orderByLatestChapter)
                : stories;
    }

    private List<HomeStorySummaryResponse> fallbackPublishedStories(
            boolean orderByLatestChapter
    ) {
        String sortColumn = orderByLatestChapter
                ? "COALESCE(latest.latest_chapter_at, s.updated_at, s.published_at)"
        ...
        return jdbc.sql("""
                        SELECT s.id,
                               ...
                               GREATEST(
                                   COALESCE(m.%s, 0),
                                   CASE
                                       WHEN '%s' = 'donation_xu' THEN COALESCE(m.view_count, 0) * 2
                                       WHEN '%s' = 'recommendation_count' THEN COALESCE(m.save_count, 0)
                                       ELSE 0
                                   END
                               ) AS metric_value
                        FROM stories s
                        ...
                        ORDER BY metric_value DESC,
                                 COALESCE(s.updated_at, s.published_at) DESC,
                                 s.title ASC
                        LIMIT 10
                        """.formatted(
                                metricColumn,

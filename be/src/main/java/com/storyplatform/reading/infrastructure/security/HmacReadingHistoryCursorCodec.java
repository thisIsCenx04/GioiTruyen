...
public final class HmacReadingHistoryCursorCodec
        implements ReadingHistoryCursorCodec {
    ...
    private static final byte[] DOMAIN =
            "gioitruyen:reading-history-cursor:v1"
                    .getBytes(StandardCharsets.UTF_8);
    private final SecretKeySpec key;

    public HmacReadingHistoryCursorCodec(byte[] rootKey) {
        Objects.requireNonNull(rootKey, "rootKey");
        if (rootKey.length < 32) {
            throw new IllegalArgumentException(
                    "reading history cursor key requires 32 bytes"
            );
        }
    ...
    public String encode(Cursor cursor) {
        ...
        String payload = String.join(
                "|",
                "v1",
                UUID.fromString(cursor.userId()).toString(),
    ...
    private byte[] sign(String value) {
        try {
            ...
            throw new IllegalStateException(
                    "cannot sign reading history cursor",
                    exception
            );
        }
    }
    ...
    private static byte[] derive(byte[] rootKey) {
        try {
            ...
            throw new IllegalStateException(
                    "cannot derive reading history cursor key",
                    exception
            );
        }
    }
    ...
    private static IllegalArgumentException invalid() {
        return new IllegalArgumentException(
                "reading history cursor is invalid"
        );
    }
}

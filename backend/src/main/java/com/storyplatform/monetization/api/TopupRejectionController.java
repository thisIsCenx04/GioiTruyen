...
@RestController
public final class TopupRejectionController {

    private final TopupRejectionOperations rejections;
    private final JwtPrivilegeEvaluator privileges;

    public TopupRejectionController(
            TopupRejectionOperations rejections,
            JwtPrivilegeEvaluator privileges
    ) {
        this.rejections = Objects.requireNonNull(rejections);
        this.privileges = Objects.requireNonNull(privileges);
    }

    @PostMapping("/admin/topups/{topupId}/reject")
    public ResponseEntity<TopupRejectionOperations.Rejection> reject(
    ...
    private static ApiException apiException(
        ...
        return new ApiException(
                status,
                "TOPUP_REJECTION_" + exception.kind(),
                "Top-up rejection failed",

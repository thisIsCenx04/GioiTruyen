  ...
  return (
    <main className={styles.reader} data-theme={night ? "night" : "paper"}>
      ...
      <nav aria-label="Đường dẫn" className={styles.breadcrumbs}>
        <Link href="/"><Home aria-hidden="true" /> Trang chủ</Link>
        <span>›</span>
        <Link href={`/truyen/${storySlug}` as Route}>{storyTitle}</Link>
        <span>›</span>
      ...
      <article className={styles.manuscript}
        ...
        <nav aria-label="Điều hướng chương" className={styles.chapterNav}>
          {chapter.previous ? (
            <Link href={`/truyen/${storySlug}/chuong-${chapter.previous.number}` as Route}>
              <ChevronLeft aria-hidden="true" /> Chương trước
            </Link>
          ) : <span />}
          <Link className={styles.storyLink} href={`/truyen/${storySlug}` as Route}>Danh sách chương</Link>
          {chapter.next ? (
            <Link href={`/truyen/${storySlug}/chuong-${chapter.next.number}` as Route}>

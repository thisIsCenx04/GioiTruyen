...
export function PublishingWorkspace({
  ...
  const api = useMemo(() => createBrowserPublishingClient({ baseUrl: API_BASE_URL, fetchImplementation: authedFetch }), []);
  ...
  async function deleteChapter(chapterId: string) {
    if (!selected || !confirm("Bạn có chắc muốn xóa chương này?")) return;
    try {
      await api.deleteChapter(teamId, selected.id, chapterId);
      ...
      await loadChapters(selected);
      setNotice("Đã xóa chương thành công.");
    } catch (requestError) {
      setError(errorMessage(requestError));
    }
  }
  ...
  async function deleteStory() {
    if (!selected || !confirm(`Bạn có chắc muốn xóa bản thảo "${selected.title}"?`)) return;
    try {
      await api.deleteStory(teamId, selected.id);
      setSelected(null);
      setStoryDraft(null);
      await reload();
      setNotice("Đã xóa bản thảo.");
    } catch (requestError) {
      setError(errorMessage(requestError));
    }
  }
  ...
  return (
    <main className={styles.workspace}>
      <aside className={styles.ledger}>
        <Link to={`/teams/${teamId}` as string}>
          <BrandMark inverse />
        </Link>
              ...
              <section className={styles.storySheet}>
                ...
                <label>
                  ...
                  <input
                    ...
                    maxLength={500}
                    onChange={(event) =>
                      editStory({ coverAssetId: event.target.value || null })
                    }
                    placeholder="Nhập URL hoặc mã ảnh bìa..."
                    value={storyDraft.coverAssetId ?? ""}
                  />
                </label>
                <label>
                  Tiến độ nội dung

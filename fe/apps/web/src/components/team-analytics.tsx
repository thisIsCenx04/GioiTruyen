  ...
  useEffect(() => {
    ...
    client.analytics(teamId, period).then(
      ...
      (error) => {
        if (!active) return;
        setReport(null);
        setState(error instanceof StoryApiError && error.problem.status === 403 ? "forbidden" : "error");
      },
    );
    return () => {
      active = false;
  ...
  if (state === "forbidden") {
    return (
      <main className={styles.messagePage}>
        ...
        <Link to={`/teams/${teamId}` as string}>Trở về hồ sơ nhóm</Link>
      </main>
    );
  }
  ...
  return (
    <main className={styles.analytics}>
      <header className={styles.header}>
        <div>
          <Link to={`/teams/${teamId}` as string}>← Không gian nhóm</Link>
